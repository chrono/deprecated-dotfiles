...
# TODO:
# - need beter way to find terminal size
...
For help see: http://wouter.coekaerts.be/irssi/nicklist.html
...
sub read_settings {
	...
	#($prefix_status[$STATUS_NORMAL] = Irssi::settings_get_str('nicklist_prefix_status_normal')) =~ s/\\e/\033/g;
	#($prefix_status[$STATUS_JOINING] = Irssi::settings_get_str('nicklist_prefix_status_joining')) =~ s/\\e/\033/g;
	#($prefix_status[$STATUS_PARTING] = Irssi::settings_get_str('nicklist_prefix_status_parting')) =~ s/\\e/\033/g;
	#($prefix_status[$STATUS_QUITING] = Irssi::settings_get_str('nicklist_prefix_status_quiting')) =~ s/\\e/\033/g;
	#($prefix_status[$STATUS_KICKED] = Irssi::settings_get_str('nicklist_prefix_status_kicked')) =~ s/\\e/\033/g;
	#($prefix_status[$STATUS_SPLIT] = Irssi::settings_get_str('nicklist_prefix_status_split')) =~ s/\\e/\033/g;
...
eval {
   ...
   do 'asm/ioctls.ph';      # without this reloading doesn't work. workaround for some unknown bug
};
# dg-hack(tm) (constant for linux only?)
...
sub calc_text {
	...
	(my $text = $nick->{'nick'}) =~ s/^(.{$tmp})..+$/$1\033[34m~/;
	#$nick->{'text'} = $prefix_mode[$nick->{'mode'}] . $prefix_status[$nick->{'status'}] . $text . (' ' x ($nicklist_width-length($nick->{'nick'})-1));
...
sub find_insert_pos {
	...
	for (my $i=0;$i < @nicklist; $i++) {
		if ($nicklist[$i]->{'cmp'} gt $cmp) {
			#print "insert: $cmp before " . $nicklist[$i]->{'cmp'};
			return $i;
		}
	}
...
sub make_nicklist {
	#my $nicklist_width = Irssi::settings_get_int('nicklist_width');
	@nicklist = ();
	$scroll_pos = 0;
...
#Irssi::settings_add_str('nicklist', 'nicklist_prefix_status_normal', '');
#Irssi::settings_add_str('nicklist', 'nicklist_prefix_status_joining', '->');
#Irssi::settings_add_str('nicklist', 'nicklist_prefix_status_parting', '<-');
#Irssi::settings_add_str('nicklist', 'nicklist_prefix_status_quiting', '<=');
#Irssi::settings_add_str('nicklist', 'nicklist_prefix_status_kicked', '<*');
#Irssi::settings_add_str('nicklist', 'nicklist_prefix_status_split', '??');
